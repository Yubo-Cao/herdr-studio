...
describe("update helpers", () => {
  ...
  test("checks the package matching a Darwin arm64 standalone binary", async () => {
    ...
    expect(await response.json()).toMatchObject({
      ...
      platform: "darwin-arm64",
      source_url:
        "https://github.com/Yubo-Cao/herdr-studio/releases/latest/download/herdr-gui-darwin-arm64.tar.xz",
    });
  ...
  test("keeps Linux x64 update checks on the Linux archive", async () => {
    ...
    expect(await response.json()).toMatchObject({
      ...
      platform: "linux-x64",
      source_url:
        "https://github.com/Yubo-Cao/herdr-studio/releases/latest/download/herdr-gui-linux-x64.tar.xz",
    });

...
describe("service definition rendering", () => {
  test("escapes systemd paths without quoting them", () => {
    const definition = renderSystemdService(
      ...
      "/home/user name/.config/herdr-gui/herdr-gui.env",
    );

    expect(definition).toContain(
      "EnvironmentFile=-/home/user\\x20name/.config/herdr-gui/herdr-gui.env",
    );
  ...
  test("passes launchd paths as arguments without interpolating shell code", () => {
    const definition = renderLaunchdService("/Applications/Herdr & GUI", {
      config: "/Users/a & b/.config/herdr-gui/herdr-gui.env",
      definition: "/unused.plist",
      stdoutLog: "/Users/a & b/Library/Logs/out.log",
      stderrLog: "/Users/a & b/Library/Logs/error.log",
    });
    ...
    expect(definition).toContain(
      "<string>/Users/a &amp; b/.config/herdr-gui/herdr-gui.env</string>",
    );
...
describe("service commands", () => {
  test("installs and starts a systemd user service", () => {
    ...
    const code = runServiceCommand(["service", "install"], {
      runtime: {
        platform: "linux",
        homeDir,
        execPath: "/home/test/.local/bin/herdr-gui",
        argv: ["/home/test/.local/bin/herdr-gui", "service", "install"],
        uid: 1000,
      },
    ...
    expect(readFileSync(definitionPath, "utf8")).toContain(
      "ExecStart=/home/test/.local/bin/herdr-gui",
    );
    ...
    expect(logs.join("\n")).toContain(
      `LAN: http://192.168.1.23:8787/?token=${token}`,
    );
  });
  ...
  test("reloads the systemd definition before restarting", () => {
    ...
    const code = runServiceCommand(["service", "reload"], {
      runtime: {
        ...
        homeDir,
        execPath: "/home/test/.local/bin/herdr-gui",
        argv: ["/home/test/.local/bin/herdr-gui", "service", "reload"],
      },
      runCommand: (argv) => {
        commands.push(argv);
        return 0;
  ...
  test("replaces a stale custom command that does not invoke this binary", () => {
    ...
    writeFileSync(
      ...
      `# Generated by herdr-gui service install.\n` +
        ...
        `/home/test/.local/bin/herdr-gui-old\n`,
    );
  ...
  test("validates launchd identity before writing service files", () => {
    ...
    const code = runServiceCommand(["service", "install"], {
      runtime: {
        platform: "darwin",
        homeDir,
        execPath: "/Users/test/.local/bin/herdr-gui",
        argv: ["/Users/test/.local/bin/herdr-gui"],
      },
      runCommand: () => {
  ...
  test("keeps a managed systemd definition when stopping fails", () => {
    ...
    const runtime = {
      ...
      homeDir,
      execPath: "/home/test/.local/bin/herdr-gui",
      argv: ["/home/test/.local/bin/herdr-gui"],
    };
    expect(
      runServiceCommand(["service", "install"], {
        runtime,
